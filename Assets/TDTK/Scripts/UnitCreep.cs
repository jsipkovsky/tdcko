...
namespace TDTK{
	...
	public class UnitCreep : Unit {
		...
		public void NextWaypoint(){
			...
			if(!reverse){
				...
				if(subWpIdx>=subPath.Count){
					...
					if(wpIdx>=path.GetWPCount()){
						...
						if(path.IsEnd()){
							...
							if (path.name.Contains("C"))
							{
								try
								{
									wpIdx = int.Parse(path.name.Substring(6));
                                    if (int.Parse(path.name.Substring(4, 1)) == 1)
                                    {
                                        wpIdx = path.nextPathP[GameHandler.shift];
                                    }
									else if (int.Parse(path.name.Substring(4, 1)) == 2)
									{
                                        var idx = GameHandler.shift2;
					...
					{
						if (path.connectorPoints.Contains(wpIdx - 1))
                        {
							int id = wpIdx;
                            //                    if (path.name.StartsWith("Path1")) {
                            //var idx = GameHandler.shift;
                            //if (idx == 1)
                            //                        {
                            //	id = wpIdx == 12 ? 27 : 12;
                            //}
						...
						if (wpIdx == path.waypointTList.Count)
						{
							path = GameObject.Find("Path10").GetComponent<Path>();
		...
		private void ReachDestination(){
			...
			// lost life only in castle
			if (path.name.Contains("4C"))
			{
