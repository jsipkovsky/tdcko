...
namespace TDTK{
				...
				if(cursorUp && !UI.IsCursorOnUI()){
					if(sInfo.HasValidPoint() && sInfo.AvailableForBuild() && sInfo.CanBuildTower(dndTower.prefabID)){
						if(RscManager.HasSufficientRsc(dndTower.GetCost())){
							...
							dndTower.Build(false, sInfo.platform);	dndTower=null;	dndCooldown=Time.unscaledTime;
	...
	public class SelectInfo{
		...
		public bool IsOccupied(){
			if(GameHandler.IsMoving) return true;
			if(platform==null || nodeID<0 || GameHandler.IsMoving) return false;
